...
func Learn(text io.Reader) {
	for t := range extractTrigrams(text) {
		storage.AddTrigram(t)
	}
}

// extractTrigrams scans and emits trigrams from the input reader
func extractTrigrams(input io.Reader) chan *trigram.Trigram {
	var words []string
	trigramC := make(chan *trigram.Trigram)
	go func(c chan *trigram.Trigram) {
		for word := range scanTrigrams(input) {
...
func scanTrigrams(input io.Reader) chan string {
